...
async function buildVendorDependencies({
  ...
  for (const [packageName, ranges] of Object.entries(VENDOR_IMPORTS)) {
    ...
    for (const [subpath, relativeEntryPoint] of Object.entries(subpaths)) {
      ...
      const specifier = path__default.default.join(packageName, subpath), chunkName = path__default.default.join(packageName, path__default.default.relative(packageName, specifier) || "index");
      entry[chunkName] = entryPoint, imports[specifier] = path__default.default.join("/", VENDOR_DIR, `${chunkName}.mjs`);
    }
  }
